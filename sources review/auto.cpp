...
// Just to remove compiler warns abut unused variables.
// Replacement [[maybe_unused]] from C++17
...
void motivation() {
  ...
  // Just an iterator
  std::vector<std::list<std::map<int, std::string>>>::const_iterator citer =
      someStrangeVector.cbegin();

  // for cycle
  for (std::vector<std::list<std::map<int, std::string>>>::const_iterator ci =
           someStrangeVector.cbegin();
       ci != someStrangeVector.cend(); ++ci) {
    // do something
  }

  // of cource, we may use using from C++11 (or typedef for legacy)
  using CIter =
      std::vector<std::list<std::map<int, std::string>>>::const_iterator;
  ...
  for (auto ci = someStrangeVector.cbegin(); ci != someStrangeVector.cend();
       ++ci) {
    // do something
  }
